...
def verify_otp(payload: VerifyOTP):
    ...
    db_fs.collection('users').document(target_username).set({
        ...
        'profile_url': "",  # Defaults to empty until updated via settings dashboard
        'created_at': firestore.SERVER_TIMESTAMP
    })
...
# --- UPGRADED: Form Multipart Profile Update Endpoint ---
...
async def update_profile(
    ...
    # Process new avatar icon asset if provided
    if avatar_file:
        # Purge old avatar file out of Firebase Storage if it exists
    ...
    # Handle Username Changes & cascade update migrations across text documents
...
def delete_user_account(username: str):
    ...
    # Purge primary avatar picture out of cloud bucket
...
# --- UPGRADED: Dynamic Profile Avatar Mapping Post Dispatcher ---
@app.get("/posts")
def get_posts(limit: int = 10, offset: int = 0, username: Optional[str] = None):
    ...
    posts = []
    # Cache to avoid repeatedly fetching the same user profile icon within the collection loop
    avatar_cache = {}
    ...
    for doc in docs:
        ...
        posts.append({
            "id": doc.id,
            "username": author,
            "user_avatar": avatar_cache[author], # Appends the lightweight user avatar dynamically
